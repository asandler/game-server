...
char get_symbol(bool isBlack) {
	return (isBlack ? '&' : '*');
}

char get_opposite_symbol(bool isBlack) {
	return (isBlack ? '*' : '&');
}


bool is_row(vvc game_table, bool isHuman, int cur_row, int cur_col) {
	/*
	...
	*/
	...
	char own_symbol = get_symbol(isHuman);
	char enemy_symbol = get_opposite_symbol(isHuman);
...
bool is_column(vvc game_table, bool isHuman, int cur_row, int cur_col) {
	/*
...
bool is_diagonals(vvc game_table, bool isHuman, int cur_row, int cur_col) {
	char own_symbol = get_symbol(isHuman);
	char enemy_symbol = get_opposite_symbol(isHuman);
...
vector<pair<int, int>> get_available_spots(const vvc game_table, bool isHuman) {
	char own_symbol = get_symbol(isHuman);
	char enemy_symbol = get_opposite_symbol(isHuman);
			...
			if (game_table[row][col] == enemy_symbol) {
				for (int k = 0; k < 4; ++k) {
					...
					if (game_table[new_row][new_col] == '-') {
						...
						bool isOk = is_row(game_table, isHuman, new_row, new_col)
									|| is_column(game_table, isHuman, new_row, new_col)
									|| is_diagonals(game_table, isHuman, new_row, new_col);

						bool isRoww = is_row(game_table, isHuman, new_row, new_col);
						bool isColumnn = is_column(game_table, isHuman, new_row, new_col);
						bool isDiagonalss = is_diagonals(game_table, isHuman, new_row, new_col);
...
void draw_row(vvc &game_table, bool isBlackStep, int cur_row, int cur_col) {
	char own_symbol = get_symbol(isBlackStep);
	char enemy_symbol = get_opposite_symbol(isBlackStep);
...
void draw_column(vvc &game_table, bool isBlackStep, int cur_row, int cur_col) {
	char own_symbol = get_symbol(isBlackStep);
	char enemy_symbol = get_opposite_symbol(isBlackStep);
...
void draw_diagonals(vvc &game_table, bool isHuman, int cur_row, int cur_col) {
	char own_symbol = get_symbol(isHuman);
	char enemy_symbol = get_opposite_symbol(isHuman);
...
void do_step(vvc &game_table, bool isBlack, int row, int col) {
	char symbol = get_symbol(isBlack);
	game_table[row][col] = symbol;

	// Делаем покраску согласно правилам игры
	draw_row(game_table, isBlack, row, col);
	draw_column(game_table, isBlack, row, col);
	draw_diagonals(game_table, isBlack, row, col);
}
